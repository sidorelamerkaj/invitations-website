// ----- URL params: title, name, surname, event -----
...
const title = capitaliseFirst((params.get('title') || '').trim());
const name = capitaliseFirst((params.get('name') || '').trim());
const surname = capitaliseFirst((params.get('surname') || '').trim());
const eventName = (params.get('event') || '').trim().replace(/\b\w/g, (c) => c.toUpperCase());

const guestParts = [title, name, surname].filter(Boolean);
const guestFullName = guestParts.length ? guestParts.join(' ') : '';

const headerTagline = document.getElementById('header-tagline');
if (headerTagline && eventName) headerTagline.textContent = eventName;

const guestLine = document.getElementById('guest-line');
const guestNameEl = document.getElementById('guest-name');
if (guestLine && guestNameEl && guestFullName) {
  guestNameEl.textContent = guestFullName;
